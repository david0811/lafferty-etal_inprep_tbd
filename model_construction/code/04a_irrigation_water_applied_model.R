...
# Approximate parameter posteriors
water_applied_soy_model_posterior <- as.data.frame(water_applied_soy_model)
colnames(water_applied_soy_model_posterior)[1] <- "intercept"

# Precip param
prcp_fit <- fitdistr(water_applied_soy_model_posterior$prcp,
                     densfun = "normal")

p_prcp <- ggplot() + 
  geom_density(data = water_applied_soy_model_posterior,
               aes(x = prcp, after_stat(density)),
               colour="red", size=1) +
  geom_function(data = data.frame(x = c(min(water_applied_soy_model_posterior$prcp),
                                        max(water_applied_soy_model_posterior$prcp))),
                aes(x = x),
                fun = dnorm,
                args = list(mean = prcp_fit$estimate['mean'],
                            sd = prcp_fit$estimate['sd']),
...
# EDD param
edd_fit <- fitdistr(water_applied_soy_model_posterior$EDD,
                    densfun = "normal")
...
p_edd <- ggplot() + 
  geom_density(data = water_applied_soy_model_posterior,
               aes(x = EDD, after_stat(density)),
               colour="red", size=1) +
  geom_function(data = data.frame(x = c(min(water_applied_soy_model_posterior$EDD),
                                        max(water_applied_soy_model_posterior$EDD))),
                aes(x = x),
                fun = dnorm,
                args = list(mean = edd_fit$estimate['mean'],
                            sd = edd_fit$estimate['sd']),
...
# Intercept
intcp_fit <- fitdistr(water_applied_soy_model_posterior$intercept,
                      densfun = "normal")
...
p_intcp <- ggplot() + 
  geom_density(data = water_applied_soy_model_posterior,
               aes(x = intercept, after_stat(density)),
               colour="red", size=1) +
  geom_function(data = data.frame(x = c(min(water_applied_soy_model_posterior$intercept),
                                        max(water_applied_soy_model_posterior$intercept))),
                aes(x = x),
                fun = dnorm,
                args = list(mean = intcp_fit$estimate['mean'],
                            sd = intcp_fit$estimate['sd']),
...
# Sigma
sigma_fit <- fitdistr(water_applied_soy_model_posterior$sigma,
                      densfun = "gamma")
...
p_sigma <- ggplot() + 
  geom_density(data = water_applied_soy_model_posterior,
               aes(x = sigma, after_stat(density)),
               colour="red", size=1) +
  geom_function(data = data.frame(x = c(min(water_applied_soy_model_posterior$sigma),
                                        max(water_applied_soy_model_posterior$sigma))),
                aes(x = x),
                fun = dgamma,
                args = list(shape = sigma_fit$estimate['shape'],
                            rate = sigma_fit$estimate['rate']),
...
ggsave('../figures/water_applied_soy_bayes_posterior.png',
       plot = (p_prcp | p_edd) / (p_intcp | p_sigma),
       width = 12, height = 6, units="in")
